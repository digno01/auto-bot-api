...
public interface InvestimentoRepository extends JpaRepository<Investimento, Long> {
    ...
    Optional<Investimento> findFirstByUsuarioAndStatusOrderByDataInicioDesc(
            User usuario,
    ...
    List<Investimento> findAllByUsuarioAndStatusOrderByDataInicioDesc(
    ...
    @Query("SELECT i FROM Investimento i " +
            ...
            "AND i.status = 'ATIVO' " +
            "AND i.dataInicio <= :dataReferencia " +
            "AND (i.dataFim IS NULL OR i.dataFim > :dataReferencia)")
    List<Investimento> findInvestimentosAtivosNaData(
            @Param("usuarioId") Long usuarioId,
            @Param("dataReferencia") LocalDateTime dataReferencia
    );

    // Busca todos os investimentos ativos em uma data específica
    @Query("SELECT i FROM Investimento i " +
            "WHERE i.status = 'ATIVO' " +
            "AND i.dataInicio <= :dataReferencia " +
            "AND (i.dataFim IS NULL OR i.dataFim > :dataReferencia)")
    List<Investimento> findAllInvestimentosAtivosNaData(
            @Param("dataReferencia") LocalDateTime dataReferencia
    );
    ...
    @Query(value = "SELECT " +
            ...
            "COALESCE(SUM(i.vl_investido), 0) as valorTotalInvestido, " +
            "COALESCE(AVG(i.nu_percentual_rendimento_diario), 0) as mediaPercentualRendimento " +
            "FROM tb_investimento i " +
            "INNER JOIN tb_robo_investidor r ON r.pk_robo_investidor = i.pk_robo_investidor " +
            "WHERE i.st_investimento = :status " +
            "AND i.dt_inicio BETWEEN :dataInicio AND :dataFim " +
            "GROUP BY r.ds_nome",
            nativeQuery = true)
    ...
    @Query("SELECT i FROM Investimento i " +
            "WHERE i.status = 'ATIVO' " +
            "AND i.dataInicio <= :dataLimite " +
            "AND i.valorUltimoRendimento IS NOT NULL " +
    ...
    @Modifying
    @Query("UPDATE Investimento i SET i.status = :novoStatus, i.dataFim = :dataFim " +
